	...
	public class DiffuseMaterial : RenderMaterial, ICyclesMaterial
	{
		...
		public override void SimulateMaterial(ref Material simulatedMaterial, RenderTexture.TextureGeneration tg)
		{
			...
			if(Diffuse.On && Diffuse.Texture!=null && DiffuseTexture.HasTextureImage) {
				simulatedMaterial.SetTexture(Diffuse.Texture.SimulatedTexture(RenderTexture.TextureGeneration.Disallow).Texture(), TextureType.Bitmap);
			}
		...
		public bool GetShader(ccl.Shader sh, bool finalize)
		{
			...
			Utilities.PbrGraphForSlot(sh, Diffuse, DiffuseTexture, diffuse.ins.Color.ToList(), false, Gamma, false, false, null);

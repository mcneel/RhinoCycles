...
namespace RhinoCyclesCore.Shaders
{
	public class RhinoFullNxt : RhinoShader
	{
		...
		static private void SetupOneDecalNodes(Shader shader, CyclesDecal decal, RhinoTextureCoordinateNode texco, ImageTextureNode imgtex, ref ISocket alpha, TextureAdjustmentTextureProcedural adjust)
		{
		...
		private (ClosureSocket decal_closure, FloatSocket mask_socket) HandleTextureDecal(List<CyclesDecal> decals, bool gamma_correct_decals, float shaderGamma)
		{
			...
			var adjustments = new List<TextureAdjustmentTextureProcedural>(count);
			int idx = 1;
			...
			for (int i = 0; i < count; i++)
			{
				...
				adjustments.Add(new TextureAdjustmentTextureProcedural(m_shader, $"Decal_texadjustment_{idx}_"));
				if (i < count - 1)
				{
			...
			else
			{
				...
				TextureAdjustmentTextureProcedural adjustA = null;
				...
				foreach (MathAdd alphaMath in alphamaths)
				{
					...
					if (idx == 0)
					{
						...
						TextureAdjustmentTextureProcedural adjustB = adjustments[idx];

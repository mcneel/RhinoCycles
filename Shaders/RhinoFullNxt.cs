...
namespace RhinoCyclesCore.Shaders
{
	public class RhinoFullNxt : RhinoShader
	{
		...
		private void SetupOneDecalNodes(CyclesDecal decal, TextureCoordinateNode texco, ImageTextureNode imgtex, MathMultiply transp)
		{
			...
			imgtex.Translation.z = 0;
			imgtex.Translation.w = 1;
			imgtex.Scale.x = decal.Texture.Transform.y.x;
			imgtex.Scale.y = decal.Texture.Transform.y.y;
			...
			switch(decal.Projection) {
				case Rhino.Render.DecalProjection.Forward:
					imgtex.Direction = TextureNode.DecalDirection.Forward;
					break;
				case Rhino.Render.DecalProjection.Backward:
					imgtex.Direction = TextureNode.DecalDirection.Backward;
					break;
				default:
					imgtex.Direction = TextureNode.DecalDirection.Both;
					break;
			}
			...
			switch(decal.Mapping) {
				...
				case Rhino.Render.DecalMapping.Cylindrical:
					imgtex.Projection = TextureNode.TextureProjection.Tube;
					break;
				case Rhino.Render.DecalMapping.Spherical:
					imgtex.Projection = TextureNode.TextureProjection.Sphere;
					break;
			}

			imgtex.DecalHeight = decal.Height;
			imgtex.DecalRadius = decal.Radius;
			imgtex.HorizontalSweepStart = decal.HorizontalSweepStart;
			imgtex.HorizontalSweepEnd = decal.HorizontalSweepEnd;
			imgtex.VerticalSweepStart = decal.VerticalSweepStart;
			imgtex.VerticalSweepEnd = decal.VerticalSweepEnd;

			transp.ins.Value2.Value = 1.0f - decal.Transparency;
			imgtex.outs.Alpha.Connect(transp.ins.Value1);

			texco.outs.Object.Connect(imgtex.ins.Vector);
			texco.outs.DecalForward.Connect(imgtex.ins.DecalForward);
			texco.outs.DecalInside.Connect(imgtex.ins.DecalInside);
		}
		...
		private ShaderNode GetShaderPart(ShaderBody part)
		{
			...
			if (part.IsPbr)
			{
				...
				var emissive = new EmissionNode("pbr_emission_");
				...
				{
					var displacement = new DisplacementNode("displacement_map_for_pbr_");
